...
module.exports = function (connect) {
    ...
    function CassandraCqlStore(options) {
        ...
        this.client.execute(cql, [], function (err) {
            if (err && err.message.indexOf(existingColumnFamilyException) === -1) {
                ...
                // Some kind of error have occurd while trying to connect to cassandra
                throw err;
            }
        });
    ...
    CassandraCqlStore.prototype.get = function (sid, callback) {
        this.client.executeAsPrepared(this.getSession, [sid], this.readConsistency, function (err, result) {
            if (err) {
                return callback(err);
            }
    ...
    CassandraCqlStore.prototype.set = function (sid, session, callback) {
        ...
        this.client.executeAsPrepared(cql, [session, sid], this.writeConsistency, callback);
    };
    ...
    CassandraCqlStore.prototype.destroy = function (sid, callback) {
        this.client.executeAsPrepared(this.deleteSession, [sid], this.writeConsistency, callback);
    };
